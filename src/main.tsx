...
function App() {
  ...
  const recordsButton = useRef<HTMLButtonElement>(null);
  ...
  useEffect(() => {
    music.player.configure(!settings.sound, settings.musicVolume);
  }, [music.player, settings.sound, settings.musicVolume]);
  function playRecord(id: string) {
    music.player.configure(!settings.sound, settings.musicVolume);
    music.player.play(id); // Synchronous user-click call; no effect or await.
    setSettings((current) => ({ ...current, music: true }));
  }
  function toggleMusic() {
    music.player.configure(!settings.sound, settings.musicVolume);
    const enabled = music.player.toggle(); // Calls audio.play() in this click stack.
    setSettings((current) => ({ ...current, music: enabled }));
  }
  useEffect(() => {
    save("gb-run-v1", run);
  }, [run]);
        ...
        <>
          ...
          <label className="setting-row">
            <span>
              ...
              <small>Jazz from Pixabay and StockTune</small>
            </span>
            <input
          ...
          <p className="fine">
            {music.isOn
              ? `Now playing · ${tracks.find((track) => track.id === music.currentTrackId)?.title}`
  ...
  return (
    <div
      className={`app ${settings.motion ? "" : "reduced-motion"} ${screen} ${recordsOpen ? "records-open" : ""}`}
    >
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          <div className="music-controls">
            <button
              className={`edition music-toggle ${music.isOn ? "music-on" : ""}`}
              aria-label={music.isOn ? "Music On" : "Play Music"}
              aria-pressed={music.isOn}
              onClick={toggleMusic}
            >
              <i aria-hidden="true" />
              {music.isOn ? "Music On" : "Play Music"}
            </button>
            <button
              ref={recordsButton}
